...
export default function SettingsBookMinistryPanel({ notify: notifyProp }) {
  ...
  const loadAll = async () => {
    setLoading(true)
    try {
      const [statsRes, settingsRes, linkedRes, borrowingRes, readingRes, permsRes, rulesRes, requestsRes, studentsRes] = await Promise.allSettled([
  ...
  return (
    <div className="flex flex-col gap-4">
      {notice ? (
        <div className="rounded-[16px] bg-[#EEF2FF] border border-[#C7D2FE] text-[#3730A3] px-4 py-2.5 text-sm font-semibold flex items-center gap-2 shadow-sm">
          <span className="h-2 w-2 rounded-full bg-[#4F46E5] animate-pulse shrink-0" />
          {notice}
        </div>
      ) : null}

      {/* Header — indigo bento 24px */}
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-1.5 p-1.5 rounded-2xl bg-white border border-slate-200 shadow-sm w-fit max-w-full">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-[13px] font-semibold transition-all duration-200 border cursor-pointer ${
                active ? 'bg-[#4F46E5] text-white border-[#4338CA] shadow-md shadow-indigo-200' : 'bg-transparent text-slate-600 border-transparent hover:bg-slate-50 hover:text-slate-900'
              }`}
            >
          ...
          {activeTab === 'linked' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <span className="chip !bg-[#EEF2FF] !text-[#4338CA] !border-[#C7D2FE] !rounded-full text-xs border px-2.5 py-1">{linkedAccounts.length} linked</span>
              </div>
              <DataTable
                data={linkedAccounts}
                rowKey="id"
                ...
                columns={[
                  ...
                  { id: 'student_status', header: 'Status', accessor: 'student_status', filterType: 'select', filterOptions: ['Active', 'Graduating', 'Graduated'], cell: (a) => statusBadge(a.student_status, { Active: 'emerald', Graduating: 'sky', Graduated: 'gold' }) },
                  { id: 'external_account_id', header: 'External Account ID', accessor: 'external_account_id', cell: (a) => <code className="text-xs font-mono bg-[#EEF2FF] border border-[#C7D2FE] text-[#4338CA] px-1.5 py-0.5 rounded-lg">{a.external_account_id}</code> },
          ...
          {activeTab === 'borrowing' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <select className="select w-auto !rounded-xl !border-slate-200 bg-white text-sm font-medium border px-3 py-1.5 rounded-lg" value={borrowStudentFilter} onChange={(e) => setBorrowStudentFilter(e.target.value)}>
              ...
              <DataTable
                ...
                columns={[
                  ...
                  { id: 'status', header: 'Status', accessor: 'status', filterType: 'select', filterOptions: ['borrowed', 'returned', 'overdue', 'lost'], cell: (b) => statusBadge(b.status, { borrowed: 'sky', returned: 'emerald', overdue: 'rose', lost: 'slate' }) },
                ]}
              />
            </div>
          )}
          ...
          {activeTab === 'reading' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <select className="select w-auto !rounded-xl !border-slate-200 bg-white text-sm font-medium border px-3 py-1.5 rounded-lg" value={readingStudentFilter} onChange={(e) => setReadingStudentFilter(e.target.value)}>
                  <option value="">All Students</option>
                  {allStudents.map((s) => (
                    <option key={s.id} value={s.id}>{s.full_name}</option>
                  ))}
              ...
              <DataTable
                ...
                columns={[
                  ...
                  { id: 'author', header: 'Author', accessor: 'author' },
                  { id: 'progress_percentage', header: 'Progress', accessor: 'progress_percentage', sortType: 'number', cell: (r) => (
                  ...
                  { id: 'status', header: 'Status', accessor: 'status', filterType: 'select', filterOptions: ['reading', 'completed', 'paused', 'abandoned'], cell: (r) => statusBadge(r.status, { reading: 'sky', completed: 'emerald', paused: 'amber', abandoned: 'slate' }) },
                ]}
              />
            </div>
          )}
          ...
          {activeTab === 'permissions' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <span className="chip !bg-[#EEF2FF] !text-[#4338CA] !border-[#C7D2FE] !rounded-full text-xs border px-2.5 py-1">{permissions.length} total</span>
              </div>
              <DataTable
                data={permissions}
                rowKey="id"
                ...
                columns={[
                  ...
                  { id: 'permission_type', header: 'Permission', accessor: 'permission_type', cell: (p) => <code className="text-xs font-mono bg-[#EEF2FF] border border-[#C7D2FE] text-[#4338CA] px-1.5 py-0.5 rounded-lg">{p.permission_type}</code> },
                  ...
                  { id: 'is_active', header: 'Active', accessor: (p) => (p.is_active ? 'active' : 'revoked'), filterType: 'select', filterOptions: ['active', 'revoked'], cell: (p) => statusBadge(p.is_active ? 'active' : 'revoked', { active: 'emerald', revoked: 'slate' }) },
                ]}
              />
            </div>
          )}
            ...
            <div className="flex flex-col gap-4">
              ...
              <DataTable
                ...
                columns={[
                  ...
                  { id: 'can_request_books', header: 'Can Request Books', accessor: (r) => (r.can_request_books ? 'Yes' : 'No'), filterType: 'select', filterOptions: ['Yes', 'No'], cell: (r) => (r.can_request_books ? <Badge tone="emerald" dot>Yes</Badge> : <Badge tone="rose" dot>No</Badge>) },
                  { id: 'digital_access', header: 'Digital Access', accessor: (r) => (r.digital_access ? 'Yes' : 'No'), filterType: 'select', filterOptions: ['Yes', 'No'], cell: (r) => (r.digital_access ? <Badge tone="emerald" dot>Yes</Badge> : <Badge tone="rose" dot>No</Badge>) },
          ...
          {activeTab === 'requests' && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <span className="chip !bg-[#EEF2FF] !text-[#4338CA] !border-[#C7D2FE] !rounded-full text-xs border px-2.5 py-1">{bookRequests.length} requests</span>
              </div>
              <DataTable
                data={bookRequests}
                rowKey="id"
                ...
                columns={[
                  ...
                  { id: 'status', header: 'Status', accessor: 'status', filterType: 'select', filterOptions: ['pending', 'approved', 'fulfilled', 'cancelled'], cell: (r) => statusBadge(r.status, { pending: 'amber', approved: 'sky', fulfilled: 'emerald', cancelled: 'slate' }) },
                ]}
              />
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="card card-hover rounded-[24px] p-5 bg-white border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <p className="mt-3 text-[13.5px] font-medium leading-relaxed text-slate-500 max-w-[70ch]">
                  GTS Book Ministry is designed to sync with an external Book Ministry application. When the Book Ministry app is ready, it will push data into these GTS endpoints:
                </p>
              ...
              <Card title="Configuration" className="!rounded-[24px] card-hover !border-slate-200">
                <div className="space-y-4 max-w-lg">
                  ...
                  <div>
                    <p className="field-label font-semibold tracking-[-0.01em] text-sm text-slate-700 mb-1">Notification Email</p>
                    <div className="flex gap-2">
                      <input className="input !rounded-xl !border-slate-200 flex-1 border rounded-lg px-3 py-2 text-sm" defaultValue={settingVal('notification_email')} placeholder="bookministry@example.com" id="bm-notif-email" />
                      <button
                        type="button"
                        onClick={async () => {
                          const val = document.getElementById('bm-notif-email').value
                          try {
                        ...
                        className="btn btn-primary btn-sm lift gap-2 !bg-[#4F46E5] hover:!bg-[#4338CA] !rounded-xl shrink-0 px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg"
                      >
                        Save
                      </button>
                    </div>
                  ...
                  <div>
                    <p className="field-label font-semibold tracking-[-0.01em] text-sm text-slate-700 mb-1">Max Requests per Student</p>
                    <div className="flex gap-2">
                      <input type="number" min="1" max="100" className="input w-24 !rounded-xl !border-slate-200 font-mono border rounded-lg px-3 py-2 text-sm" defaultValue={settingVal('max_requests_per_student', '5')} id="bm-max-req" />
                      <button
                        type="button"
                        onClick={async () => {
                          const val = document.getElementById('bm-max-req').value
                          try {
                        ...
                        className="btn btn-primary btn-sm lift gap-2 !bg-[#4F46E5] hover:!bg-[#4338CA] !rounded-xl px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg"
                      >
                        Save
                      </button>
                    </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium leading-relaxed text-amber-800 mt-1">
                      The borrowing history, reading records, and linked accounts are intended to be populated by the Book Ministry application. When the external app is built, configure it to send data to the API endpoints listed above. Students are matched via <code className="font-mono bg-amber-100 border border-amber-200 px-1.5 py-0.5 rounded-lg text-amber-900 text-[11px]">external_account_id</code> linked to their GTS profile.
                    </p>
                  </div>
                </div>
              </div>
